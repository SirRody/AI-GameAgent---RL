...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    if terminal:
        ...
        target_q = reward + GAMMA * maxq_next
    
    # Calculate current Q-value for the taken action-object pair
    current_q_action = q_values_action_cur[action_index]
    current_q_object = q_values_object_cur[object_index]
    current_q = 1 / 2 * (current_q_action + current_q_object)
    
    # Calculate MSE loss
    loss = F.mse_loss(current_q, torch.tensor(target_q, dtype=torch.float32))
