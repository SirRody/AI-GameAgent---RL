...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        # Extract Q-values for current state
        state_q_values = q_func[state_1, state_2, :, :]
        
        # Find the indices of the maximum Q-value
        # np.argmax returns flat index, np.unravel_index converts to 2D indices
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    # Q-learning update: Q_new = (1-α)Q_old + α*target
    new_q = (1 - ALPHA) * current_q + ALPHA * target
    
    # Update Q-table
    q_func[current_state_1, current_state_2, action_index, object_index] = new_q
...
def run_episode(for_training):
    ...
    # Start game
    current_room, current_quest, terminal = framework.newGame()
    current_s1 = dict_room_desc[current_room]
    current_s2 = dict_quest_desc[current_quest]
    
    while not terminal:
        # Choose and execute action
        a_idx, o_idx = epsilon_greedy(current_s1, current_s2, q_func, epsilon)
        next_room, next_quest, reward, terminal = framework.step_game(
            current_room, current_quest, a_idx, o_idx
        )
        
        # Convert next state
        next_s1 = dict_room_desc[next_room]
        next_s2 = dict_quest_desc[next_quest]
        
        if for_training:
            # Update Q-function
            tabular_q_learning(
                q_func, current_s1, current_s2, a_idx, o_idx,
                reward, next_s1, next_s2, terminal
            )
        ...
        # Move to next state
        current_room, current_quest = next_room, next_quest
