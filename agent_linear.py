...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    q_values = theta @ state_vector  # Vector of Q-values for all actions
    max_index = np.argmax(q_values)  # Index of maximum Q-value
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if terminal:
        ...
        # Only compute max if not terminal
        max_next_q = np.max(theta @ next_state_vector)
...
def run_episode(for_training):
    ...
    # For testing: track discounted reward
    epi_reward = 0.0
    discount = 1.0
    ...
    while not terminal:
        # Combine room and quest for feature extraction
        ...
        if for_training:
            ...
            # Update theta using linear Q-learning
            linear_q_learning(
